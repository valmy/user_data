...
def main(
    ...
    # Get available pairs
    whitelist = set(config["exchange"]["pair_whitelist"]) if "exchange" in config and "pair_whitelist" in config["exchange"] else set()
    ...
    default_pair = "ETH/USDT:USDT" if "ETH/USDT:USDT" in available_pairs else (available_pairs[0] if available_pairs else "ETH/USDT:USDT")
    ...
    if serve:
        ...
        # Set initial visible window to last ~6 hours (120 candles @ 3m)
        try:
            import pandas as pd
            x_values = df_plot["date"] if "date" in df_plot.columns else df_plot.index
            if len(x_values) >= 2:
                # Compute end as last timestamp, start as 120th from last (or first if shorter)
                end = x_values.iloc[-1]
                start_idx = max(0, len(x_values) - 120)
        ...
        def _on_pair_or_view_change(pair_value, relayout_data, current_fig, table_children):  # type: ignore
            try:
                ...
                if triggered.endswith("relayoutData") and current_fig and current_fig.get("data") and relayout_data:
                    x0 = x1 = None
                ...
                fig2 = go.Figure(
                    data=[
                        go.Candlestick(
                            ...
                            open=df_plot2["open"], high=df_plot2["high"],
                            low=df_plot2["low"], close=df_plot2["close"],
                            increasing_line_color="lime", increasing_fillcolor="lime",
                ...
                # Initial range: last ~6 hours and y from visible
                x_vals2 = df_plot2["date"] if "date" in df_plot2.columns else df_plot2.index
                if len(x_vals2) >= 2:
                    end2 = x_vals2.iloc[-1]
                    start2 = x_vals2.iloc[max(0, len(x_vals2) - 120)]
                ...
                return current_fig, dmc.Alert(title="Error", children=str(e), color="red", variant="filled")
